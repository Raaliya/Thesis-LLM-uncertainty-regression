...
OUTPUT_DIR = os.path.join(ROOT, "outputs_exp01_mcd_baseline_skincare")
...
MODEL_PATH = os.path.join(OUTPUT_DIR, "mcd_baseline_model.keras")
SCALER_PATH = os.path.join(OUTPUT_DIR, "mcd_baseline_scaler.joblib")
IMPUTER_PATH = os.path.join(OUTPUT_DIR, "mcd_baseline_imputer.joblib")
PREDICTIONS_CSV = os.path.join(OUTPUT_DIR, "mcd_baseline_predictions.csv")
METRICS_JSON = os.path.join(OUTPUT_DIR, "mcd_baseline_metrics.json")
FEATURES_TXT = os.path.join(OUTPUT_DIR, "mcd_baseline_features.txt")
...
if len(feature_cols) == 0:
    raise ValueError("No numeric feature columns found for baseline experiment.")
...
print("Training MCD baseline model...")
...
mc_predictions = np.array(mc_predictions)   # shape: [mc_passes, n_samples]
...
metrics = {
    "experiment": "Exp-01 Baseline MCD",
    "dataset": "skincare",
...
with open(FEATURES_TXT, "w", encoding="utf-8") as f:
    f.write("MCD Baseline Features Used\n")
    f.write("=" * 40 + "\n")
    for col in feature_cols:
        f.write(f"{col}\n")
