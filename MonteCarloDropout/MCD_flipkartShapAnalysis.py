# mcd_flipkart_shap.py
# Monte Carlo Dropout + SHAP for Flipkart dataset
# Target: rating
# This version drops the requested columns and renames feature columns
# by adding 'llm_' prefix where needed for display consistency.
...
FILE_NAME = "flipkart_merged_original_withllm_features.csv"   # change if needed
TARGET_COL = "rating"
...
# Output files
MODEL_FILE = os.path.join(FILE_PATH, "mcd_flipkart_model.pt")
SCALER_FILE = os.path.join(FILE_PATH, "mcd_flipkart_scaler.joblib")
PREDICTIONS_FILE = os.path.join(FILE_PATH, "mcd_flipkart_predictions.csv")

BAR_PLOT_FILE = os.path.join(FILE_PATH, "mcd_flipkart_shap_bar.png")
BEESWARM_PLOT_FILE = os.path.join(FILE_PATH, "mcd_flipkart_shap_beeswarm.png")
WATERFALL_PLOT_FILE = os.path.join(FILE_PATH, "mcd_flipkart_shap_waterfall.png")
...
def find_actual_target_column(df, requested_target):
    """
    Find the target column in a case-insensitive way.
    """
...
def add_llm_prefix_if_needed(col_name, target_col):
    """
    Add llm_ prefix to feature columns for display consistency,
    but do not rename the target column.
    """
...
def mc_predict(model, X_tensor, mc_samples=50, device="cpu"):
    """
    Return predictive mean and predictive std using Monte Carlo dropout.
    """
    model.to(device)
    model.eval()
...
def predict_from_raw_for_shap(X_raw, model, scaler, mc_samples=50, device="cpu"):
    """
    SHAP passes raw numpy arrays. Scale them before prediction.
    Return predictive mean only.
    """
...
# After renaming, refresh actual target column name
