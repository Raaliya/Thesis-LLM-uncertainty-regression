import os
import re
import json
import hashlib
from typing import Dict, Any, Optional, Tuple

import numpy as np
import pandas as pd
import requests
from tqdm import tqdm

# =========================================================
# Women Clothing Reviews | Exp02 | LLM Feature Extraction
# Ollama local | JSON-safe | Debug + sanity checks
# =========================================================

# -------------------------
# CONFIG
# -------------------------
CSV_PATH = "women_clothing_reviews_normalized.csv"  # <-- change if different
OUT_DIR = "outputs_exp02_women_clothing_ollama_2000"
os.makedirs(OUT_DIR, exist_ok=True)

CACHE_PATH = os.path.join(OUT_DIR, "ollama_cache.jsonl")
FEATURES_OUT = os.path.join(OUT_DIR, "women_clothing_llm_features_2000.csv")
BAD_ROWS_OUT = os.path.join(OUT_DIR, "bad_rows_debug.jsonl")
SAMPLE_RAW_OUT = os.path.join(OUT_DIR, "sample_raw_responses.jsonl")

OLLAMA_BASE = os.environ.get("OLLAMA_URL", "http://localhost:11434")
OLLAMA_MODEL = os.environ.get("OLLAMA_MODEL", "llama3.1:latest")

N_ROWS = 2000
RANDOM_STATE = 42

# speed + stability
MAX_CHARS = 300
TEMPERATURE = 0.0
NUM_PREDICT = 260          # IMPORTANT: allow enough tokens for full JSON
CONNECT_TIMEOUT = 10
READ_TIMEOUT = 900         # 15 minutes for slow rows

PRINT_EVERY = 10
SAVE_RAW_EVERY = 50        # save a few raw samples to prove Ollama is being called

# -------------------------
# THEMES (10-theme codebook, consistent with other datasets)
# -------------------------
THEMES = [
    "fit_sizing",
    "fabric_material_quality",
    "comfort_wearability",
    "style_appearance",
    "color_accuracy",
    "durability_after_wash",
    "value_for_money",
    "shipping_delivery",
    "return_exchange_experience",
    "recommendation_intent"
]

SYSTEM_INSTRUCTIONS = f"""
You are a qualitative researcher performing DEDUCTIVE qualitative coding using a fixed codebook.

CRITICAL RULES:
- Only code what is explicitly stated in the text. Do NOT guess.
- If a theme is not mentioned: present=0, polarity=0, intensity=0.
- Polarity: -1 negative, 0 neutral/mixed/unclear, +1 positive.
- Intensity: 0 none, 1 mild, 2 moderate, 3 strong.
- Output MUST be valid JSON ONLY. No extra text. No markdown. No explanations.

CODEBOOK THEMES:
1) fit_sizing
2) fabric_material_quality
3) comfort_wearability
4) style_appearance
5) color_accuracy
6) durability_after_wash
7) value_for_money
8) shipping_delivery
9) return_exchange_experience
10) recommendation_intent

GLOBAL:
- overall_sentiment: -1 / 0 / +1
- confidence: 0.0–1.0

REQUIRED JSON SCHEMA (exact keys):
{{
  "overall_sentiment": -1|0|1,
  "confidence": 0.0-1.0,
  "themes": {{
    "{THEMES[0]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}},
    "{THEMES[1]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}},
    "{THEMES[2]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}},
    "{THEMES[3]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}},
    "{THEMES[4]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}},
    "{THEMES[5]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}},
    "{THEMES[6]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}},
    "{THEMES[7]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}},
    "{THEMES[8]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}},
    "{THEMES[9]}": {{"present":0|1,"polarity":-1|0|1,"intensity":0-3}}
  }}
}}
""".strip()

# -------------------------
# Helper functions
# -------------------------
def append_jsonl(path: str, obj: Any) -> None:
    with open(path, "a", encoding="utf-8") as f:
        f.write(json.dumps(obj, ensure_ascii=False) + "\n")

def load_cache(path: str) -> Dict[str, Any]:
    cache = {}
    if os.path.exists(path):
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                obj = json.loads(line)
                cache[obj["key"]] = obj["value"]
    return cache

def row_hash(title, review_text) -> str:
    return hashlib.sha256((str(title) + "||" + str(review_text)).encode("utf-8", errors="ignore")).hexdigest()

def extract_json_block(text: str) -> Optional[str]:
    text = (text or "").strip()
    if not text:
        return None
    if text.startswith("{") and text.endswith("}"):
        return text
    m = re.search(r"\{.*\}", text, re.DOTALL)
    return m.group(0) if m else None

def clamp_int(v, lo, hi, default=0):
    try:
        v = int(v)
        return max(lo, min(hi, v))
    except Exception:
        return default

def clamp_float(v, lo=0.0, hi=1.0, default=0.5):
    try:
        v = float(v)
        return max(lo, min(hi, v))
    except Exception:
        return default

def zero_schema() -> Dict[str, Any]:
    return {
        "overall_sentiment": 0,
        "confidence": 0.0,
        "themes": {t: {"present": 0, "polarity": 0, "intensity": 0} for t in THEMES}
    }

def normalize(parsed: Dict[str, Any]) -> Dict[str, Any]:
    out = {
        "overall_sentiment": clamp_int(parsed.get("overall_sentiment", 0), -1, 1, 0),
        "confidence": clamp_float(parsed.get("confidence", 0.5), 0.0, 1.0, 0.5),
        "themes": {}
    }
    themes = parsed.get("themes", {})
    if not isinstance(themes, dict):
        themes = {}

    for theme in THEMES:
        t = themes.get(theme, {})
        if not isinstance(t, dict):
            t = {}
        present = clamp_int(t.get("present", 0), 0, 1, 0)
        polarity = clamp_int(t.get("polarity", 0), -1, 1, 0)
        intensity = clamp_int(t.get("intensity", 0), 0, 3, 0)

        if present == 0:
            polarity = 0
            intensity = 0

        out["themes"][theme] = {"present": present, "polarity": polarity, "intensity": intensity}
    return out

def make_text(title, review_text) -> str:
    t = "" if pd.isna(title) else str(title)
    r = "" if pd.isna(review_text) else str(review_text)
    combined = f"TITLE: {t}\nREVIEW: {r}".strip()
    if len(combined) > MAX_CHARS:
        combined = combined[:MAX_CHARS] + "…"
    return combined

def ollama_chat(messages) -> str:
    payload = {
        "model": OLLAMA_MODEL,
        "stream": False,
        "messages": messages,
        "options": {
            "temperature": TEMPERATURE,
            "num_predict": NUM_PREDICT
        }
    }
    r = requests.post(
        f"{OLLAMA_BASE}/api/chat",
        json=payload,
        timeout=(CONNECT_TIMEOUT, READ_TIMEOUT)
    )
    r.raise_for_status()
    data = r.json()
    return (data.get("message", {}) or {}).get("content", "")

def strict_code(text_in: str) -> Tuple[Dict[str, Any], str, str]:
    # primary
    user_msg = (
        "Return ONLY valid JSON for the schema given. "
        "No prose, no markdown, no backticks.\n\n"
        f"TEXT:\n{text_in}"
    )
    raw1 = ollama_chat([
        {"role": "system", "content": SYSTEM_INSTRUCTIONS},
        {"role": "user", "content": user_msg}
    ])

    j1 = extract_json_block(raw1)
    if j1:
        try:
            return normalize(json.loads(j1)), raw1, ""
        except json.JSONDecodeError:
            pass

    # repair
    repair_msg = (
        "Convert the following text into STRICT VALID JSON matching the required schema. "
        "Return JSON only. If something is missing, use zeros.\n\n"
        f"TEXT TO FIX:\n{raw1}"
    )
    raw2 = ollama_chat([
        {"role": "system", "content": SYSTEM_INSTRUCTIONS},
        {"role": "user", "content": repair_msg}
    ])

    j2 = extract_json_block(raw2)
    if j2:
        try:
            return normalize(json.loads(j2)), raw1, raw2
        except json.JSONDecodeError:
            pass

    # fail-safe
    return zero_schema(), raw1, raw2

def flatten(codes: Dict[str, Any]) -> Dict[str, float]:
    feats = {
        "llm_overall_sentiment": float(codes["overall_sentiment"]),
        "llm_confidence": float(codes["confidence"]),
    }
    for theme, v in codes["themes"].items():
        feats[f"llm_{theme}_present"] = float(v["present"])
        feats[f"llm_{theme}_polarity"] = float(v["polarity"])
        feats[f"llm_{theme}_intensity"] = float(v["intensity"])
    return feats

def detect_columns(df: pd.DataFrame) -> Tuple[str, str]:
    colmap = {c.lower(): c for c in df.columns}

    # women dataset common variants
    title_col = colmap.get("title", None)

    review_col = (
        colmap.get("review text")
        or colmap.get("review_text")
        or colmap.get("review")
        or colmap.get("text")
        or colmap.get("reviewtext")
    )

    if title_col is None:
        # Title sometimes missing in some versions; treat as blank
        title_col = "__NO_TITLE__"
        df[title_col] = ""

    if review_col is None:
        raise ValueError(
            "Could not detect review text column. Expected something like 'Review Text' or 'Review'. "
            f"Available columns: {list(df.columns)}"
        )
    return title_col, review_col

def main():
    df = pd.read_csv(CSV_PATH)

    # sample exactly 2000 if possible
    if len(df) >= N_ROWS:
        df = df.sample(n=N_ROWS, random_state=RANDOM_STATE).reset_index(drop=True)
    else:
        print(f"[WARN] Only {len(df)} rows available. Using all rows.")
        df = df.reset_index(drop=True)

    title_col, review_col = detect_columns(df)

    print("Using model:", OLLAMA_MODEL)
    print("Using base :", OLLAMA_BASE)
    print("Title col  :", title_col)
    print("Review col :", review_col)
    print("Rows used  :", len(df))

    cache = load_cache(CACHE_PATH)
    print("Loaded cache entries:", len(cache))

    rows = []

    for i, row in enumerate(tqdm(df.itertuples(index=False), total=len(df))):
        title = getattr(row, title_col) if hasattr(row, title_col) else ""
        review_text = getattr(row, review_col) if hasattr(row, review_col) else ""

        key = row_hash(title, review_text)

        if key in cache:
            codes = cache[key]
        else:
            text_in = make_text(title, review_text)

            if i == 0:
                print("\n[DEBUG] First text sent to Ollama:\n", text_in, "\n")

            codes, raw1, raw2 = strict_code(text_in)

            # Save raw samples periodically (proves Ollama is being called)
            if (i % SAVE_RAW_EVERY) == 0:
                append_jsonl(SAMPLE_RAW_OUT, {
                    "i": i,
                    "text_in": text_in,
                    "raw1": raw1,
                    "raw2": raw2
                })

            # If schema is all zeros, log it (so it doesn't silently happen)
            if sum(v["present"] for v in codes["themes"].values()) == 0 and codes["overall_sentiment"] == 0:
                append_jsonl(BAD_ROWS_OUT, {
                    "i": i,
                    "reason": "all_zero_schema_returned",
                    "text_in": text_in,
                    "raw1": raw1,
                    "raw2": raw2
                })

            append_jsonl(CACHE_PATH, {"key": key, "value": codes})
            cache[key] = codes

        rows.append({
            "row_index": i,
            **flatten(codes)
        })

        if (i % PRINT_EVERY) == 0:
            print(f"Processed {i+1}/{len(df)} rows")

    out_df = pd.DataFrame(rows)
    out_df.to_csv(FEATURES_OUT, index=False)

    llm_cols = [c for c in out_df.columns if c.startswith("llm_")]
    nonzero_cells = int((out_df[llm_cols] != 0).sum().sum())
    nonzero_rows = int((out_df[llm_cols].sum(axis=1) != 0).sum())

    print("\nDONE")
    print("Saved to:", FEATURES_OUT)
    print("Sanity check:")
    print(" - Nonzero cells:", nonzero_cells)
    print(" - Rows with any nonzero:", nonzero_rows)
    print("Debug files:")
    print(" - Raw samples:", SAMPLE_RAW_OUT)
    print(" - Any all-zero rows logged:", BAD_ROWS_OUT)
    print(" - Cache:", CACHE_PATH)

    if nonzero_cells == 0:
        print("\n[ALERT] All LLM features are zero. Check:")
        print("1) Is the review column detected correctly?")
        print("2) Open sample_raw_responses.jsonl to see what Ollama returned.")
        print("3) Increase NUM_PREDICT (e.g., 320) if JSON is truncated.")

if __name__ == "__main__":
    main()
