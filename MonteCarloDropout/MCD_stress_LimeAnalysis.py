...
MODEL_PATH = os.path.join(WORK_DIR, "mcd_stress_model.pt")
SCALER_PATH = os.path.join(WORK_DIR, "mcd_stress_scaler.joblib")
OUTPUT_HTML = os.path.join(WORK_DIR, "mcd_stress_lime_dashboard.html")
PRED_PATH = os.path.join(WORK_DIR, "mcd_stress_predictions.csv")
...
# rename remaining feature columns with llm_ prefix
rename_map = {}
for col in df.columns:
...
# confidence target likely 0-1, so meaningful bands adapted
p_lt_03 = float(np.mean(mc_preds < 0.3))
p_03_to_07 = float(np.mean((mc_preds >= 0.3) & (mc_preds < 0.7)))
p_ge_07 = float(np.mean(mc_preds >= 0.7))
...
<div class="card">
    ...
    {build_probability_bar("Confidence < 0.3", p_lt_03, "#d9d9d9")}
    {build_probability_bar("0.3 ≤ Confidence < 0.7", p_03_to_07, "#ffbf00")}
    {build_probability_bar("Confidence ≥ 0.7", p_ge_07, "#ff7f0e")}
</div>
"""
...
<head>
<meta charset="utf-8">
<title>MCD Stress Analysis – Single-Instance Explanation Dashboard</title>
<style>
    body {{
...
<body>
    <div class="page">
        <h1>MCD Stress Analysis – Single-Instance Explanation Dashboard</h1>
        <div class="subtitle">
