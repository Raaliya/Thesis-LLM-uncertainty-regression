# ============================================================
# SHAP Explanation for Flipkart Dataset - Bayesian Neural Network
# Generates: Bar Plot, Beeswarm Plot, Waterfall Plot, Combined Figure
# Baseline/original features + best 15 LLM-derived features
# ============================================================
...
DATASET_PATH = r"D:\MAT-UNI\SEMESTER 02\THESIS\VS Code Implementation 1.2\1 BNN_ShapLime\flipkart_merged_original_withllm_features.csv"

OUTPUT_DIR = r"D:\MAT-UNI\SEMESTER 02\THESIS\VS Code Implementation 1.2\1 BNN_ShapLime\SHAP_BNN_Flipkart"
...
# Important:
# The target column is listed in DROP_COLUMNS, but it must not be removed
# from the dataframe before model training.
...
if len(baseline_features) == 0:
    print("\nWarning: No baseline/original numeric features were found after dropping metadata columns.")
...
print("\nFinal features used for Flipkart BNN + SHAP:")
...
print("\nTraining Bayesian Neural Network model...")
...
@torch.no_grad()
def predict_scaled(X_scaled, sample=False):
    model.eval()
...
def predict_original_scale_from_raw(X_input_raw):
    ...
    preds_scaled = predict_scaled(X_scaled, sample=False)
    preds_original = y_scaler.inverse_transform(preds_scaled)

    return preds_original.ravel()


@torch.no_grad()
def predict_bnn_distribution_single(X_input_raw_single, n_samples=100):
    ...
    for _ in range(n_samples):
        ...
        preds.append(pred_original.flatten()[0])
...
bar_path = os.path.join(OUTPUT_DIR, "flipkart_bnn_shap_bar.png")
beeswarm_path = os.path.join(OUTPUT_DIR, "flipkart_bnn_shap_beeswarm.png")
waterfall_path = os.path.join(OUTPUT_DIR, "flipkart_bnn_shap_waterfall.png")
combined_path = os.path.join(OUTPUT_DIR, "flipkart_bnn_shap_combined.png")
...
fig.suptitle(
    "Figure: SHAP-based explanation for Flipkart dataset using Bayesian Neural Network model",
    fontsize=18,
    y=0.02
)
...
feature_list_path = os.path.join(
    OUTPUT_DIR,
    "selected_features_for_flipkart_bnn_shap.csv"
)
...
# ============================================================
# 17. SAVE LOCAL PREDICTION SUMMARY
# ============================================================

local_raw = X_test.iloc[[waterfall_index]]
local_distribution = predict_bnn_distribution_single(local_raw, n_samples=100)

local_summary = pd.DataFrame({
    "actual_rating": [float(y_test[waterfall_index][0])],
    "predicted_mean": [float(np.mean(local_distribution))],
    "predictive_std": [float(np.std(local_distribution))],
    "lower_95_pi": [float(np.mean(local_distribution) - 1.96 * np.std(local_distribution))],
    "upper_95_pi": [float(np.mean(local_distribution) + 1.96 * np.std(local_distribution))]
})

local_summary_path = os.path.join(
    OUTPUT_DIR,
    "flipkart_bnn_local_prediction_summary.csv"
)

local_summary.to_csv(local_summary_path, index=False)

print("\nLocal prediction summary saved:")
