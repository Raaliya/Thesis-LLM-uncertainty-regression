...
DATA_PATH_CANDIDATES = [
    os.path.join(WORK_DIR, "stress_llm_features_2000.csv"),
    os.path.join(WORK_DIR, "reviews_1000_1500_with_llm_features_2000.csv"),
    os.path.join(WORK_DIR, "stress_llm_features_2000"),
    os.path.join(WORK_DIR, "reviews_1000_1500_with_llm_features_2000"),
]

OUTPUT_HTML = os.path.join(
    WORK_DIR,
    "de_stress_lime_dashboard_top15_llm_original_features_with_prefix.html"
)
...
# =========================================================
# ORIGINAL STRESS / LIWC FEATURES TO DISPLAY WITHOUT llm_ PREFIX
# =========================================================
...
# =========================================================
# LOAD VALID STRESS DATASET
# =========================================================
DATA_PATH = None
df_original_full = None

for candidate in DATA_PATH_CANDIDATES:
    if os.path.exists(candidate):
        temp_df = pd.read_csv(candidate, low_memory=False)

        has_target = TARGET_COL in temp_df.columns
        has_original_features = all(c in temp_df.columns for c in ORIGINAL_FEATURES_REQUIRED)

        if has_target and has_original_features:
            DATA_PATH = candidate
            df_original_full = temp_df
            break

if DATA_PATH is None:
    raise FileNotFoundError(
        "No valid Stress dataset was found.\n\n"
        "The script checked these files:\n"
        + "\n".join(DATA_PATH_CANDIDATES)
        + "\n\nA valid Stress dataset must contain:\n"
        + f"- target column: {TARGET_COL}\n"
        + "- original LIWC features:\n"
        + "\n".join(ORIGINAL_FEATURES_REQUIRED)
    )
...
def display_name(feature_name: str) -> str:
    """
    Adds llm_ prefix only for display.
    The actual dataset/model/scaler feature name is not changed.
    """
    if feature_name in ORIGINAL_FEATURES_REQUIRED:
        return feature_name
...
def extract_actual_feature_from_rule(rule: str, actual_features: list[str], display_features: list[str]) -> str:
...
def force_prefix_in_html(lime_html: str, actual_features: list[str]) -> str:
    for feat in sorted(actual_features, key=len, reverse=True):
        shown = display_name(feat)

        if shown != feat:
            lime_html = lime_html.replace(feat, shown)
...
# =========================================================
# AUTO-FIND COMPATIBLE DE STRESS MODEL + SCALER
# =========================================================
...
if not scaler_candidates:
    raise FileNotFoundError(
        "No compatible DE Stress scaler was found.\n"
        "The scaler must include the six LIWC features and the Stress LLM features.\n"
        "If no such scaler exists, retrain the DE Stress model using LIWC + LLM features."
    )

if not model_candidates:
    raise FileNotFoundError(
        "No compatible Deep Ensembles model file was found. "
        "The model file must contain one or more state_dicts with 'net.0.weight'."
    )
...
print("\nSelected DE Stress scaler:")
print(SCALER_PATH)

print("\nSelected DE Stress model:")
print(MODEL_PATH)
...
if missing_original_from_dataset:
    raise ValueError(
        "The following original Stress LIWC features are missing from the dataset:\n"
...
if missing_original_from_model:
    raise ValueError(
        "The following original Stress LIWC features exist in the dataset but are NOT "
...
print("\nDeep Ensemble Stress model loaded successfully.")
...
def predict_from_display(display_array, display_actual_features):
    ...
    full_raw = np.tile(base_instance_full_raw, (display_array.shape[0], 1))

    for i, actual_feature in enumerate(display_actual_features):
        pos = model_feature_names.index(actual_feature)
        full_raw[:, pos] = display_array[:, i]
...
# =========================================================
all_display_actual_features = [
    c for c in model_feature_names
    if c not in ALWAYS_EXCLUDE
]

llm_candidate_features = [
    c for c in all_display_actual_features
    if is_llm_feature(c)
]

if len(llm_candidate_features) == 0:
    raise ValueError(
        "No LLM-derived Stress features were detected in the selected DE model/scaler."
    )

full_selection_actual_features = ORIGINAL_FEATURES_REQUIRED + llm_candidate_features

full_selection_actual_features = [
    c for c in full_selection_actual_features
    if c in model_feature_names
]

full_selection_display_features = [
    display_name(c) for c in full_selection_actual_features
]

X_lime_full_raw = X_model_raw[full_selection_actual_features].copy()

full_explainer = LimeTabularExplainer(
    training_data=X_lime_full_raw.values,
    feature_names=full_selection_display_features,
    mode="regression",
...
full_exp = full_explainer.explain_instance(
    ...
    predict_fn=lambda x: predict_from_display(x, full_selection_actual_features),
    num_features=len(full_selection_actual_features),
    num_samples=LIME_NUM_SAMPLES
)

llm_contributions = {}

for rule, contribution in full_exp.as_list():
    ...
    if actual_feature in llm_candidate_features:
        llm_contributions[actual_feature] = abs(float(contribution))
...
compact_actual_features = []

for c in ORIGINAL_FEATURES_REQUIRED + top_llm_features:
    if c in model_feature_names and c not in compact_actual_features:
        compact_actual_features.append(c)

compact_display_features = [
    display_name(c) for c in compact_actual_features
]

NUM_FEATURES_FINAL = len(compact_actual_features)

print("\nFeatures displayed in final dashboard:")
for actual, shown in zip(compact_actual_features, compact_display_features):
    print(f" - {shown}")

# =========================================================
# STEP 2: FINAL COMPACT LIME EXPLANATION
# =========================================================
X_lime_compact_raw = X_model_raw[compact_actual_features].copy()
...
print("\nGenerating compact DE Stress LIME explanation...")

exp = compact_explainer.explain_instance(
    data_row=X_lime_compact_raw.iloc[INSTANCE_INDEX].values,
    predict_fn=lambda x: predict_from_display(x, compact_actual_features),
...
p_lt_030 = float(np.mean(ensemble_preds < 0.30))
p_030_to_060 = float(np.mean((ensemble_preds >= 0.30) & (ensemble_preds < 0.60)))
p_ge_060 = float(np.mean(ensemble_preds >= 0.60))
...
for rule, contribution in exp.as_list():
    ...
    if actual_feature in instance_raw_display.index and actual_feature not in used_features:
        raw_value = instance_raw_display[actual_feature]

        table_rows.append({
            "feature": display_name(actual_feature),
...
if not table_rows:
    for feat in compact_actual_features:
        table_rows.append({
...
lime_html_raw = force_prefix_in_html(lime_html_raw, compact_actual_features)
...
<head>
<meta charset="utf-8">
<title>DE Stress Analysis LIME Dashboard</title>
...
<body>
    <h1>DE Stress Analysis – Single-Instance Explanation Dashboard</h1>
...
print("\nDE Stress LIME dashboard saved to:")
print(OUTPUT_HTML)
